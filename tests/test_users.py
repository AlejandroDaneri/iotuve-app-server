...
class UsersTestCase(unittest.TestCase):
    ...
    @patch('src.clients.auth_api.requests.post')
    def test_post_user_without_username_should_return_bad_request(self, mock_post):
    ...
    @patch('src.clients.auth_api.requests.post')
    def test_post_user_without_password_should_return_bad_request(self, mock_post):
        mock_post.side_effect = Exception("should not call auth api post")
        r = self.app.post('/api/v1/users', json=dict(
            username="testuser",
            first_name="test",
    ...
    @patch('src.clients.auth_api.requests.post')
    def test_post_user_without_email_should_return_bad_request(self, mock_post):
        mock_post.side_effect = Exception("should not call auth api post")
        r = self.app.post('/api/v1/users', json=dict(
            username="testuser",
            password="password",
    ...
    @patch('src.clients.auth_api.requests.post')
    def test_post_user_with_invalid_email_should_return_bad_request(self, mock_post):
        mock_post.side_effect = Exception("should not call auth api post")
        r = self.app.post('/api/v1/users', json=dict(
            username="testuser",
            password="password",
    ...
    def test_patch_invalid_format_should_return_bad_request(self, mock_patch, mock_session):
    ...
    @patch('src.clients.auth_api.AuthAPIClient.get_session')
    @patch('src.clients.auth_api.requests.patch')
    def test_patch_invalid_op_should_return_bad_request(self, mock_patch, mock_session):
        mock_patch.side_effect = Exception("should not call auth api patch")
        mock_session.return_value.json.return_value = dict(username="testuser")
        mock_session.return_value.status_code = HTTPStatus.OK
        r = self.app.patch(
